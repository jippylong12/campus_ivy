...
RSpec.describe CampusIvy::Resources::DisbursementBatchDetail do
  let(:client) { CampusIvy::Client.new(token: 'test_api_key') }
  let(:resource) { client.disbursement_batch_detail }
  let(:base_url) { 'https://api.campusivy.com/v1' }

  describe '#list' do
    it 'requests list' do
      stub_request(:get, "#{base_url}/DisbursementBatchDetail")
